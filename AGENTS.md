...
| 문제 | 원인 | 해결책 |
|------|------|--------|
| **홈 버튼 누락** | 새 파일 생성 시 홈 버튼 코드를 빼먹음 | 체크리스트 2️⃣ 항목 반드시 확인. 위 템플릿 코드 복사 사용 |
| **CSS 호환성 경고** | `-webkit-line-clamp`만 사용 | 표준 `line-clamp` 속성도 함께 추가 필요 |

### CSS line-clamp 올바른 사용법
```css
.line-clamp-3 {
    display: -webkit-box;
    -webkit-line-clamp: 3;
    line-clamp: 3;           /* 표준 속성 추가 필수! */
